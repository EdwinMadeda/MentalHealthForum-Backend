...
import static com.mentalhealthforum.mentalhealthforum_backend.contants.VerificationTokenConstants.*;
        ...
        return tokenRepository.deleteByEmailAndType(email, type) // Only kill same-type tokens
                .then(Mono.defer(() -> {
                    VerificationTokenEntity newToken = new VerificationTokenEntity(
                            ...
                            email,
                            Instant.now().plus(TOKEN_EXPIRY_DURATION_HOURS),
                            type,
                            groupPath,
                            newValue
                    );
    ...
    public Mono<Void> checkRateLimit(String email){
        ...
        return  tokenRepository.findByEmail(normalizedEmail)
                .flatMap(existingToken -> {
                    ...
                    if(existingToken.getCreatedAt().isAfter(Instant.now().minus(TOKEN_RATE_LIMIT_DURATION_MINUTES))){
                        return Mono.error(
                                new TooManyRequestsException(
                                        String.format("Please wait at least %d minutes before requesting another verification email.",
                                                TOKEN_RATE_LIMIT_MINUTES)));
                    }
